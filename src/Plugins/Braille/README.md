...
> **PDF snapshots** of this document and the [user guide](UserGuide.md) are attached to
> [GitHub releases](https://github.com/opensheetmusicdisplay/opensheetmusicdisplay/releases)
> rather than committed to the repository (recommitting binaries on every documentation edit
> would bloat the git history). Regenerate them with `bin/braille/generate_braille_doc_pdfs.py`;
> after demo UI changes, refresh the user guide's screenshots first with
> `bin/braille/generate_braille_doc_screenshots.mjs`. The markdown files here are always current.
...
To try it interactively: `npm start`, open the demo, and load one of the **"Music Braille
Test"** samples at the bottom of the sample dropdown — selecting one switches the braille
display on automatically, and the braille output and its translation table appear below the
score. For any other score, enable **"Show Music Braille score (below classical)"** in the
collapsed **"Music Braille options"** section at the bottom of the left panel, or open the
demo with `?braille=1`. The remaining `test_Braille_*` files in `test/data` can be loaded
via the "Open file..." button or `?braille=1&openUrl=test_Braille_ScaleSimple.musicxml&endUrl`.
The [hosted public braille demo](https://opensheetmusicdisplay.org/wp-content/uploads/sites/2/osmd-braille-demo/index.html?braille=1)
linked in the user guide is exactly this: the standard OSMD demo opened with `?braille=1`.
...
The demo (`npm start`, source `demo/index.js` — everything braille lives in the "Music
Braille" section at the bottom of the file, entry point `renderBraille`) treats braille as an
opt-in feature: the master toggle **"Show Music Braille score (below classical)"** (off by
default, in the collapsed "Music Braille options" section at the bottom of the sidebar) turns
the braille display on — as do the "Music Braille Test" samples and the `?braille=1` URL
parameter. While enabled, the demo converts the loaded sheet after every load and whenever
one of the braille checkboxes changes (facsimile, bar-over-bar, ensemble, lyrics — all off by
default so standard braille music is what users see first). No score re-render is needed for
option changes; with braille off, the converter is never even instantiated. **"Show classical
score (above braille)"** (on by default, only effective while braille is shown) hides the
visual score and — more importantly — skips `render()` entirely for subsequently loaded
scores: `load()` alone builds the data model the converter reads, which makes large scores
load much faster. Facsimile mode is the exception: it reads the rendered `GraphicSheet`, so
the demo renders (hidden via CSS, which preserves the container width) whenever facsimile is
active.
