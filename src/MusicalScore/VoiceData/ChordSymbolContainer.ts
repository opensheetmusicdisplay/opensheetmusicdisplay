...
export class ChordSymbolContainer {
  ...
  public static calculateChordText(
    chordSymbol: ChordSymbolContainer,
    ...
    if (MusicSheetCalculator.transposeCalculator) {
      transposedRootPitch =
        MusicSheetCalculator.transposeCalculator.transposePitch(
    ...
    // main Note
    let text: string = Pitch.getNoteEnumString(
      transposedRootPitch.FundamentalNote
    );
    // main alteration
    ...
    // chord kind text
    let chordKind: string = chordSymbol.getTextFromChordKindEnum(
      chordSymbol.ChordKind
    );
