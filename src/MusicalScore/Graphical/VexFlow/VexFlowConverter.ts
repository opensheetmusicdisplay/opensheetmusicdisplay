...
interface LyricLookaheadInfo {
    candidates: {
        ...
        isSemanticContinuation: boolean;
    }[];
}
...
export class VexFlowConverter {
    ...
    public static StaveNote(gve: GraphicalVoiceEntry): VF.StaveNote {
        ...
        if (gve.parentVoiceEntry.IsGrace || gve.notes[0].sourceNote.IsCueNote) {
            ...
            const lyricsEntries: GraphicalLyricEntry[] = gve.parentStaffEntry.LyricsEntries.filter(
                (entry: GraphicalLyricEntry) => entry.LyricsEntry.Parent === gve.parentVoiceEntry,
            );
            ...
            if (rules.RenderLyrics &&
                ...
                const hasVoiceEntry: (staffEntry: GraphicalStaffEntry) => boolean =
                    (staffEntry: GraphicalStaffEntry): boolean =>
                    staffEntry.graphicalVoiceEntries.some(
                        (voiceEntry: GraphicalVoiceEntry) =>
                            voiceEntry?.parentVoiceEntry?.ParentVoice ===
                            gve.parentVoiceEntry.ParentVoice &&
                            !voiceEntry.parentVoiceEntry.IsGrace,
                    );
                const hasRhythmicEntry: (staffEntry: GraphicalStaffEntry) => boolean =
                    (staffEntry: GraphicalStaffEntry): boolean =>
                    staffEntry.graphicalVoiceEntries.some(
                        (voiceEntry: GraphicalVoiceEntry) =>
                ...
                const isFirstVoiceEntryInMeasure: boolean = !measureStaffEntries
                    .slice(0, currentStaffEntryIndex)
                    .some(hasVoiceEntry);
                const isFirstRhythmicStaffEntryInMeasure: boolean = !measureStaffEntries
                    .slice(0, currentStaffEntryIndex)
                    .some(hasRhythmicEntry);
                const isLastNoteInMeasure: boolean = !measureStaffEntries
                    .slice(currentStaffEntryIndex + 1)
                    .some(hasRhythmicEntry);
                ...
                // The first lyric-bearing note of a measure is created after the
                // preceding measure's last note. Complete the adjacent-column
                // lookahead across the barline here, while both VexFlow notes can
                // still contribute to their measures' minimum widths. A scalar
                // elongation of the following measure cannot move its first note.
                let crossMeasureRemainingPaddingPx: number = 0;
                if (isFirstVoiceEntryInMeasure &&
                    isFirstRhythmicStaffEntryInMeasure &&
                    rules.LyricsXPaddingForLastNoteInMeasure) {
                    const musicSheet: MusicSheet =
                        currentStaffEntry.parentMeasure.ParentStaff.ParentInstrument.GetMusicSheet;
                    const currentMeasureIndex: number =
                        currentStaffEntry.parentMeasure.parentSourceMeasure.measureListIndex;
                    const previousGraphicalMeasure: GraphicalMeasure =
                        musicSheet.SourceMeasures[currentMeasureIndex - 1]?.VerticalMeasureList?.find(
                            (measure: GraphicalMeasure) =>
                                measure?.ParentStaff === currentStaffEntry.parentMeasure.ParentStaff,
                        );
                    const previousStaffEntry: GraphicalStaffEntry =
                        [...(previousGraphicalMeasure?.staffEntries ?? [])]
                            .reverse()
                            .find(hasRhythmicEntry);
                    const previousVoiceEntry: GraphicalVoiceEntry =
                        previousStaffEntry?.graphicalVoiceEntries.find(
                            (voiceEntry: GraphicalVoiceEntry) =>
                                voiceEntry?.parentVoiceEntry?.ParentVoice ===
                                gve.parentVoiceEntry.ParentVoice &&
                                !voiceEntry.parentVoiceEntry.IsGrace &&
                                !!(voiceEntry as any).vfStaveNote,
                        );
                    const previousLyricsEntries: GraphicalLyricEntry[] =
                        previousStaffEntry?.LyricsEntries.filter(
                            (entry: GraphicalLyricEntry) =>
                                entry.LyricsEntry.Parent === previousVoiceEntry?.parentVoiceEntry,
                        ) ?? [];
                    if (previousVoiceEntry && previousLyricsEntries.length > 0) {
                        let previousHasShortNotes: boolean = false;
                        for (const previousNote of previousVoiceEntry.notes) {
                            if (previousNote.sourceNote.Length.RealValue <= 0.125) {
                                previousHasShortNotes = true;
                                break;
                            }
                        }
                        let crossMeasureRightPadding: number = 0;
                        for (const previousLyricEntry of previousLyricsEntries) {
                            const previousIdentity: string = previousLyricEntry.getLineIdentity();
                            const candidates: GraphicalLyricEntry[] = lyricsEntries.filter(
                                (entry: GraphicalLyricEntry) =>
                                    entry.getLineIdentity() === previousIdentity ||
                                    entry.LyricsEntry.IsChorus !== previousLyricEntry.LyricsEntry.IsChorus,
                            );
                            if (candidates.length === 0) {
                                continue;
                            }
                            const previousFootprint: LyricFootprint = previousLyricEntry.getFootprint();
                            const hasDash: boolean = previousLyricEntry.hasDashFromLyricWord();
                            const dashWidth: number = hasDash ? previousLyricEntry.getDashWidth() : 0;
                            const previousRightExtent: number = previousFootprint.rightExtent + dashWidth;
                            let requiredRightAllowance: number = previousRightExtent;
                            for (const candidate of candidates) {
                                const isSemanticContinuation: boolean =
                                    candidate.getLineIdentity() === previousIdentity;
                                const minSpacing: number = hasDash && isSemanticContinuation
                                    ? rules.BetweenSyllableMinimumDistance
                                    : rules.HorizontalBetweenLyricsDistance;
                                requiredRightAllowance = Math.max(
                                    requiredRightAllowance,
                                    previousRightExtent + candidate.getFootprint().leftExtent + minSpacing,
                                );
                            }
                            ...
                            if (!previousHasShortNotes) {
                                availableAllowance += rules.LyricsXPaddingReductionForLongNotes;
                            }
                            crossMeasureRightPadding = Math.max(
                                crossMeasureRightPadding,
                                requiredRightAllowance - availableAllowance,
                            );
                        }
                        if (crossMeasureRightPadding > 0) {
                            const previousColumnRightPaddingPx: number = Math.max(
                                0,
                                ...previousStaffEntry.graphicalVoiceEntries
                                    .filter((voiceEntry: GraphicalVoiceEntry) =>
                                        !voiceEntry?.parentVoiceEntry?.IsGrace &&
                                        !!(voiceEntry as any).vfStaveNote,
                                    )
                                    .map((voiceEntry: GraphicalVoiceEntry) =>
                                        Number(
                                            (voiceEntry as any).vfStaveNote
                                                .getLayoutPadding()
                                                .rightPx ?? 0,
                            ...
                            crossMeasureRemainingPaddingPx = Math.max(
                                0,
                                10 * rules.LyricsXPaddingFactorForLongLyrics * crossMeasureRightPadding -
                                    previousColumnRightPaddingPx,
                            );
                        }
                    }
                }

                let nextLyricEntriesInVoice: GraphicalLyricEntry[] = [];
                for (let seIndex: number = currentStaffEntryIndex + 1;
                    seIndex < measureStaffEntries.length;
                    seIndex++) {
                    const nextStaffEntry: GraphicalStaffEntry = measureStaffEntries[seIndex];
                    const nextVoiceEntry: GraphicalVoiceEntry = nextStaffEntry.graphicalVoiceEntries.find(
                        (voiceEntry: GraphicalVoiceEntry) =>
                            voiceEntry?.parentVoiceEntry?.ParentVoice ===
                            gve.parentVoiceEntry.ParentVoice &&
                            !voiceEntry.parentVoiceEntry.IsGrace,
                    ...
                    nextLyricEntriesInVoice = nextStaffEntry.LyricsEntries.filter(
                        (entry: GraphicalLyricEntry) =>
                            entry.LyricsEntry.Parent === nextVoiceEntry.parentVoiceEntry,
                    );
                    break;
                }
                ...
                for (const lyricsEntry of lyricsEntries) {
                    ...
                    const candidates: LyricLookaheadInfo["candidates"] = nextLyricEntriesInVoice
                        .filter((entry: GraphicalLyricEntry) =>
                            entry.getLineIdentity() === lineIdentity ||
                            entry.LyricsEntry.IsChorus !== lyricsEntry.LyricsEntry.IsChorus,
                        )
                        .map((entry: GraphicalLyricEntry) => ({
                            entry,
                            isSemanticContinuation: entry.getLineIdentity() === lineIdentity,
                        }));
                    // Padding is local to consecutive rhythmic entries in this voice.
                    // A semantic role hand-off (for example, verse 2 to a chorus
                    // centred on verse 2's row) can still share visible horizontal
                    // space. Ordinary sparse verse lines remain the measured residual
                ...
                for (const lyricsEntry of lyricsEntries) {
                    ...
                    if (lookahead?.candidates.length === 0 && !isLastNoteInMeasure) {
                        if (!isFirstRhythmicStaffEntryInMeasure) {
                            continue;
                        }
                    }
                    ...
                    for (const candidate of lookahead?.candidates ?? []) {
                        const nextFootprint: LyricFootprint = candidate.entry.getFootprint();
                        const minSpacing: number = hasDash && candidate.isSemanticContinuation
                            ? rules.BetweenSyllableMinimumDistance
                            : rules.HorizontalBetweenLyricsDistance;
                        requiredRightAllowance = Math.max(
                            requiredRightAllowance,
                            currentRightExtent + nextFootprint.leftExtent + minSpacing,
                        );
                    }
                ...
                const normalLeftPaddingPx: number =
                    10 * rules.LyricsXPaddingFactorForLongLyrics * Math.max(0, leftPadding);
                const crossMeasureAdditionalLeftPaddingPx: number = Math.max(
                    0,
                    crossMeasureRemainingPaddingPx - normalLeftPaddingPx,
                );
                if (normalLeftPaddingPx > 0 || rightPadding > 0 || crossMeasureAdditionalLeftPaddingPx > 0) {
                    vfnote.setLayoutPadding(
                        normalLeftPaddingPx +
                            crossMeasureAdditionalLeftPaddingPx,
