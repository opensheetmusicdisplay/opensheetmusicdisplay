...
export class SkyBottomLineCalculator {
    ...
    public calculateLines(): void {
        ...
        for (const measure of this.StaffLineParent.Measures as VexFlowMeasure[]) {
            ...
            // VexFlow's CanvasContext doesn't expose getImageData, so we use the native context through vexFlowCanvasContext
            let imageData: any = null;
            try {
                const nativeCtx: any = (ctx as any).vexFlowCanvasContext || ctx;
                imageData = nativeCtx.getImageData(0, 0, width, height);
            } catch (e) {
                log.warn("SkyBottomLineCalculator: getImageData not available, using default skyline", e);
                // Fallback: create dummy data that will result in default positioning
