...
import {
  ...
  CustomChord,
  DegreesInfo,
} from "../../MusicalScore/VoiceData/ChordSymbolContainer";
...
export class EngravingRules {
  ...
  public NoteDistancesScalingFactors: number[] = [
  ...
  public FillEmptyMeasuresWithWholeRest:
    | FillEmptyMeasuresWithWholeRests
    | number;
  ...
  public loadDefaultValues(): void {
    ...
    this.StaffLineWidth = 0.1; // originally 0.12, but this will be pixels in Vexflow (*10).
    ...
    try {
      this.MaxInstructionsConstValue =
        this.ClefLeftMargin +
        this.ClefRightMargin +
        this.KeyRightMargin +
        this.RhythmRightMargin +
        11;
      //if (FontInfo.Info) {
  ...
  public setPreferredSkyBottomLineBackendAutomatically(
    numberOfGraphicalMeasures: number = -1
  ): void {
    let vendor: string = "";
    let userAgent: string = "";
    if (typeof globalThis === "object") {
      // it looks like globalThis can be undefined and cause build issues in es2017 (e.g. Android API 28), see #1299
    ...
    if (this.DisableWebGLInSafariAndIOS && /apple/i.test(vendor)) {
      // doesn't apply to Chrome on MacOS
      alwaysUsePlain = true;
  ...
  public resetChordAccidentalTexts(
    chordAccidentalTexts: Dictionary<AccidentalEnum, string>,
    useChordAccidentalsUnicode: boolean
  ): void {
    chordAccidentalTexts.setValue(
      AccidentalEnum.SHARP,
      useChordAccidentalsUnicode ? "♯" : "#"
    );
    chordAccidentalTexts.setValue(
      AccidentalEnum.FLAT,
      useChordAccidentalsUnicode ? "♭" : "b"
    );
    chordAccidentalTexts.setValue(
      AccidentalEnum.DOUBLEFLAT,
      useChordAccidentalsUnicode ? "𝄫" : "bb"
    );
    chordAccidentalTexts.setValue(
      AccidentalEnum.DOUBLESHARP,
      useChordAccidentalsUnicode ? "𝄪" : "x"
    );
  }
  ...
  public resetChordSymbolLabelTexts(
    chordtexts: Dictionary<ChordSymbolEnum, string>
  ): Dictionary<ChordSymbolEnum, string> {
    ...
    chordtexts.setValue(
      ChordSymbolEnum.halfdiminished,
      `m7${this.ChordAccidentalTexts.getValue(AccidentalEnum.FLAT)}5`
    );
  ...
  public addChordName(
    ...
    if (ChordSymbolEnum[chordKindText] !== undefined) {
      ...
      this.CustomChords.push(
        CustomChord.createCustomChord(
          altName,
          ChordSymbolEnum[chordKindText],
          degrees
        )
      );
    }
  ...
  public resetChordNames(): void {
    const sharp: string = this.ChordAccidentalTexts.getValue(
      AccidentalEnum.SHARP
    );
    const flat: string = this.ChordAccidentalTexts.getValue(
      AccidentalEnum.FLAT
    );
...
export class PageFormat {
  constructor(
    width: number,
    height: number,
    idString: string = "noIdStringGiven"
  ) {
    this.width = width;
    this.height = height;
  ...
  public Equals(otherPageFormat: PageFormat): boolean {
    ...
    return (
      otherPageFormat.width === this.width &&
      otherPageFormat.height === this.height
    );
  }
}
