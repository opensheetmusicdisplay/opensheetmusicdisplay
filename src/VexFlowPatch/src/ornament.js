...
const SMUFL_FONT_FAMILY = 'OSMD_Bravura';
const SMUFL_FONT_STYLE_ID = 'osmd-smufl-bravura';

function ensureSmuflFont(ctx) {
  if (typeof document === 'undefined') {
    return;
  }

  if (!document.getElementById(SMUFL_FONT_STYLE_ID)) {
    const style = document.createElement('style');
    style.id = SMUFL_FONT_STYLE_ID;
    style.textContent = `@font-face {
      font-family: '${SMUFL_FONT_FAMILY}';
  ...
  if (ctx && ctx.svg && !ctx.svg.querySelector(`style[data-osmd-smufl="${SMUFL_FONT_STYLE_ID}"]`)) {
    const svgStyle = document.createElementNS(ctx.svg.namespaceURI, 'style');
    svgStyle.setAttribute('data-osmd-smufl', SMUFL_FONT_STYLE_ID);
    ...
    ctx.svg.insertBefore(svgStyle, ctx.svg.firstChild);
  }
}
...
function renderSmuflOrnament(ornament, ctx, glyphX, glyphY) {
  ...
  ensureSmuflFont(ctx);
  ctx.save();
  ctx.setFont(SMUFL_FONT_FAMILY, fontSize, 'normal');
  const measured = ctx.measureText(ornament.ornament.smuflGlyph);
  const width = measured.width || ornament.getWidth() || fontSize;
...
export class Ornament extends Modifier {
  ...
  draw() {
    ...
    if (this.ornament.smuflGlyph) {
      glyphY -= renderSmuflOrnament(this, ctx, glyphX, glyphY);
    } else {
