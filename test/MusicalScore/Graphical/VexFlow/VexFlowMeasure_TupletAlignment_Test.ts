         ...
         for (const gve of staffEntry.graphicalVoiceEntries) {
            ...
            if (vfVoiceEntry.notes.length > 0 && vfVoiceEntry.notes[0].sourceNote) {
               ...
               if (sourceNote.NoteTuplet && vfVoiceEntry.vfStaveNote) {
                  ...
                  const expectedTicks: number = Math.round(graphicalLength.RealValue * VexFlow.RESOLUTION);
...
describe("VexFlow Measure - Cross-Staff Tuplet Alignment", () => {
   ...
   it("Should fill a cross-staff tuplet gap with a ghost note of the exact tuplet length", (done: Mocha.Done) => {
      ...
      const tripletEighthTicks: number = VexFlow.RESOLUTION / 12;
      ...
      expect(totalTicks).to.be.closeTo(VexFlow.RESOLUTION / 2, 0.001,
   ...
   it("Should align the beat-2 tuplet note with the simultaneous note in the other voice", (done: Mocha.Done) => {
      ...
      const beat2Ticks: number = VexFlow.RESOLUTION / 4;
